...
public class HighlandsEventManager {
     ...
     * onInitBiomeGenerators(WorldTypeEvent.InitBiomeGens event) { if
     * (event.worldType instanceof IMWWorldType) { //event.newBiomeGens =
     * TerrainGenInjector.assembleModdedBiomeGenerators(event.seed,
     * event.worldType); // event.newBiomeGens =
     * GenLayerBiomeHL.initializeAllBiomeGenerators(event.seed, event.worldType)
     * } Logs.log(Level.INFO, "[Highlands]
     * WorldTypeEvent.InitBiomeGens="+event.originalBiomeGens);
    ...
    public void onVillageSelectBlock(GetVillageBlockID event) {
        if (event.biome != null && HighlandsBiomes.sahel != null && HighlandsBiomes.outback != null && BiomeGenBase.icePlains != null) {
            ...
            if (event.biome.biomeName.equals(BiomeGenBase.icePlains.biomeName)) {
                ...
                if (event.original == Blocks.cobblestone) {
                    event.replacement = Blocks.cobblestone;
                }
                if (event.original == Blocks.planks) {
                ...
                if (event.original == Blocks.stone_stairs) {
                    event.replacement = Blocks.stone_stairs;
                }
                if (event.original == Blocks.gravel) {
                    event.replacement = Blocks.gravel;
                }
