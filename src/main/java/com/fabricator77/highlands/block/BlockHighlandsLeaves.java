...
public class BlockHighlandsLeaves extends BlockLeavesBase implements IShearable, IFactoryHarvestable {
     ...
     * par5, int par6) { byte b0 = 1; int j1 = b0 + 1;
     *
     * if (par1World.checkChunksExist(par2 - j1, par3 - j1, par4 - j1, par2 +
     * j1, par3 + j1, par4 + j1)) { for (int k1 = -b0; k1 <= b0; ++k1) { for
     * (int l1 = -b0; l1 <= b0; ++l1) { for (int i2 = -b0; i2 <= b0; ++i2) {
     * Block j2 = par1World.getBlock(par2 + k1, par3 + l1, par4 + i2);
     *
     * if (j2 != null) { j2.beginLeavesDecay(par1World, par2 + k1, par3 + l1,
     * par4 + i2); } } } } } }
     */
    /**
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random) {
        ...
        if (par1World.canBlockSeeTheSky(par2, par3 + 1, par4) && !World.doesBlockHaveSolidTopSurface(par1World, par2, par3 - 1, par4) && par5Random.nextInt(15) == 1) {
    ...
    public Item getItemDropped(int metadata, Random par2Random, int fortune) {
        if (treeType == 0) {
            return Item.getItemFromBlock(HighlandsBlocks.firSapling);
        }
        if (treeType == 1) {
            return Item.getItemFromBlock(HighlandsBlocks.acaciaSapling);
        }
        if (treeType == 2) {
            return Item.getItemFromBlock(HighlandsBlocks.poplarSapling);
        }
        if (treeType == 3) {
            return Item.getItemFromBlock(HighlandsBlocks.redwoodSapling);
        }
        if (treeType == 4) {
            return Item.getItemFromBlock(HighlandsBlocks.canopySapling);
        }
        if (treeType == 10) {
            return Item.getItemFromBlock(HighlandsBlocks.palmSapling);
        }
        if (treeType == 11) {
            return Item.getItemFromBlock(HighlandsBlocks.ironwoodSapling);
        }
        if (treeType == 12) {
            return Item.getItemFromBlock(HighlandsBlocks.mangroveSapling);
        }
        if (treeType == 13) {
            return Item.getItemFromBlock(HighlandsBlocks.ashSapling);
        }
        if (treeType == 14) {
            return Item.getItemFromBlock(HighlandsBlocks.autumnYellowSapling);
        }
        if (treeType == 15) {
            return Item.getItemFromBlock(HighlandsBlocks.autumnOrangeSapling);
        }
