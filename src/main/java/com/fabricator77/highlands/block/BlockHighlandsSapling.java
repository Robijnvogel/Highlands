...
import com.fabricator77.highlands.worldgen.WorldGenAutumnTree;
import com.fabricator77.highlands.worldgen.WorldGenHighlandsBigTree;
import com.fabricator77.highlands.worldgen.WorldGenHighlandsShrub;
import com.fabricator77.highlands.worldgen.WorldGenTreeAcacia;
import com.fabricator77.highlands.worldgen.WorldGenTreeAsh;
import com.fabricator77.highlands.worldgen.WorldGenTreeCanopy;
import com.fabricator77.highlands.worldgen.WorldGenTreeFir;
import com.fabricator77.highlands.worldgen.WorldGenTreeIronwood;
import com.fabricator77.highlands.worldgen.WorldGenTreeMangrove;
import com.fabricator77.highlands.worldgen.WorldGenTreePalm;
import com.fabricator77.highlands.worldgen.WorldGenTreePoplar;
import com.fabricator77.highlands.worldgen.WorldGenTreeRedwood;
...
public class BlockHighlandsSapling extends BlockSapling implements IPlantable, IFactoryFertilizable, IFactoryPlantable {
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        /**
         * if (meta < 0 || meta >= treeNames.length) { meta = 0; }
         *
    ...
    private boolean isSoilGoodForSaplingType(Block soil, World par1World, int par2, int par3, int par4) {
        if (soil.canSustainPlant(par1World, par2, par3 - 1, par4, ForgeDirection.UP, this)) {
            return true;
        }
        if (treeType == 0 && soil == Blocks.snow) {
            return true;
        }
        if (treeType == 10 && soil == Blocks.sand) {
            return true;
        }
        if (treeType == 12 && soil == Blocks.water) {
            return true;
        }
        return false;
    }
    ...
    public void growTree(World par1World, int i, int j, int k, Random r) {
        ...
        if (treeType == 0) {
            if (growWideTree) {
                ...
                isTreeGrowSuccess = new WorldGenTreeFir(10, 15, true, false).generate(par1World, r, i, j, k);
            }
        }
        if (treeType == 1) {
            isTreeGrowSuccess = new WorldGenTreeAcacia(7, 3, true).generate(par1World, r, i, j, k);
        }
        //if(treeType == 1)isTreeGrowSuccess = new WorldGenTreeDiamondheart(70, 20, true).generate(par1World, r, i, j, k);

        if (treeType == 2) {
            isTreeGrowSuccess = new WorldGenTreePoplar(10, 4, true).generate(par1World, r, i, j, k);
        }
        if (treeType == 3) {
            isTreeGrowSuccess = new WorldGenTreeRedwood(35, 10, true).generate(par1World, r, i, j, k);
        }
        if (treeType == 4) {
            if (growWideTree) {
                isTreeGrowSuccess = new WorldGenTreeCanopy(13, 6, true, true).generate(par1World, r, xysw[0], j, xysw[1]);
            } else {
                isTreeGrowSuccess = new WorldGenTreeCanopy(13, 6, true, false).generate(par1World, r, i, j, k);
            }
        }
        if (treeType == 5) {
        ...
        if (treeType == 8) {
            isTreeGrowSuccess = new WorldGenHighlandsShrub(1, 1).generate(par1World, r, i, j, k);
        }
        if (treeType == 9) {
            isTreeGrowSuccess = new WorldGenHighlandsShrub(0, 0).generate(par1World, r, i, j, k);
        }
        if (treeType == 10) {
            isTreeGrowSuccess = new WorldGenTreePalm(8, 3, true).generate(par1World, r, i, j, k);
        }
        if (treeType == 11) {
            isTreeGrowSuccess = new WorldGenTreeIronwood(25, 50, true).generate(par1World, r, i, j, k);
        }
        if (treeType == 12) {
            isTreeGrowSuccess = new WorldGenTreeMangrove(4, 2, false).generate(par1World, r, i, j, k);
        }
        if (treeType == 13) {
            isTreeGrowSuccess = new WorldGenTreeAsh(16, 8, false).generate(par1World, r, i, j, k);
        }
        if (treeType == 14) {
            isTreeGrowSuccess = new WorldGenAutumnTree(true, 4, Blocks.log, HighlandsBlocks.autumnOrangeLeaves).generate(par1World, r, i, j, k);
        }
        if (treeType == 15) {
            isTreeGrowSuccess = new WorldGenAutumnTree(true, 4, Blocks.log, HighlandsBlocks.autumnYellowLeaves).generate(par1World, r, i, j, k);
        } /*
         ...
         */ else if (!isTreeGrowSuccess && replaceSapling) {
            if (par1World.isAirBlock(i, j, k)) {
                par1World.setBlock(i, j, k, this, meta, 2);
            }
        }
    }
    ...
    public boolean canBePlantedHere(World world, int x, int y, int z, ItemStack stack) {
        return isValidPosition(world, x, y, z, stack.getCurrentDurability());
    }
    ...
    public boolean isValidPosition(World world, int x, int y, int z, int meta) {
        ...
        if (world.isAirBlock(x, y, z)) {
            ...
            if (treeType == 0 && currentBlock == Blocks.snow) {
                return true;
            }
            if (treeType == 10 && currentBlock == Blocks.sand) {
                return true;
            }
            if (treeType == 12 && currentBlock == Blocks.water) {
                return true;
            }
